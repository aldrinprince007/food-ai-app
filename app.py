...
with col2:
    food = st.text_input("🍽️ What do you want?", "biryani").lower().strip()
...
def get_restaurants(city, food):
    ...
    try:
        ...
        (
          ...
          node["amenity"="fast_food"](around:8000,{lat},{lon});
        );
        out;
        """
    ...
    for place in data:
        ...
        if food in name or food in cuisine:
            filtered.append(place)

    # 🔥 IMPORTANT FIX: If too few results → show all
    if len(filtered) < 5:
        filtered = data
